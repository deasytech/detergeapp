      ...
      <div class="container-fluid  dashboard-content">
        ...
        <div class="row">
          ...
          <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
            ...
            <div class="card">
              <div class="card-body">
                <form action="{{ route('order.update', $order->id) }}" id="basicform" data-parsley-validate="" method="post">
                  @csrf
                  {{ method_field('PUT') }}
                  ...
                  <div class="form-group">
                    <label for="vendor_id">Order to assign</label>
                    <select class="form-control" id="vendor_id" name="vendor_id" required>
                      <option></option>
                      @foreach(presentVendors() as $technician)
                        <option value="{{ $technician->id }}" {{ $technician->id == $order->vendor_id ? 'selected' : '' }}>{{ $technician->name }}</option>
                      @endforeach
                    </select>
                  </div>
                  <div class="row">
                    <div class="form-group col-md-4">
                      ...
                      <select class="form-control" id="service_type_id" name="service_type_id" required>
                        ...
                        @foreach(presentServiceTypes() as $service)
                          <option value="{{ $service->id }}" {{ $service->id == $order->service_type_id ? 'selected' : '' }}>{{ $service->name }}</option>
                        @endforeach
                      </select>
                    </div>
                    <div class="form-group col-md-4">
                      <label for="service_location">Service Location</label>
                      <input id="service_location" type="text" name="service_location" data-parsley-trigger="change" value="{{ $order->service_location }}" autocomplete="off" class="form-control">
                    ...
                    <div class="form-group col-md-4">
                      ...
                      <input type="date" name="actual_service_date" class="form-control" required value="{{ $order->actual_service_date }}">
                    </div>
                  </div>
                  <div class="row">
                    <div class="form-group col-md-4">
                      <label for="dispenser_brand">Dispenser Brand</label>
                      <input id="dispenser_brand" type="text" name="dispenser_brand" required="" class="form-control" value="{{ $order->dispenser_brand }}">
                    </div>
                    <div class="form-group col-md-4">
                      <label for="dispenser_quantity">Dispenser Quantity</label>
                      <input id="dispenser_quantity" name="dispenser_quantity" type="text" required="" class="form-control" value="{{ $order->dispenser_quantity }}">
                    </div>
                    <div class="form-group col-md-42">
                      <label for="cost">Cost</label>
                      <input id="cost" name="cost" type="text" required="" class="form-control" value="{{ $order->cost }}">
                    </div>
                    <div class="form-grou col-md-2">
                      <label for="disinfect">Disinfect</label>
                      <hr class="mt-0 mb-1">
                      <label class="custom-control custom-radio custom-control-inline">
                        <input type="radio" name="disinfect" {{ $order->disinfect == 1 ? 'checked' : '' }} value="1" class="custom-control-input">
                        <span class="custom-control-label">Yes</span>
                      </label>
                      <label class="custom-control custom-radio custom-control-inline">
                        <input type="radio" name="disinfect" {{ $order->disinfect == 0 ? 'checked' : '' }} value="0" class="custom-control-input">
                  ...
                  <div class="form-group">
                    ...
                    <textarea id="problem_description" name="problem_description" required="" rows="4" class="form-control">{{ $order->problem_description }}</textarea>
                  </div>
                  <div class="row">
